...
describe("User controller", () => {
    ...
    it('Empty username to throw validation error', async () => {
        await expect(UserController.createUser({email:"", username:""})).rejects.toEqual(
            new BaseError("Username is invalid", ErrorCodes.INVALID_USERNAME)
        );
    });
    it('Empty email to throw validation error', async () => {
        await expect(UserController.createUser({email:"", username:"test"})).rejects.toEqual(
            new BaseError("This email is invalid", ErrorCodes.INVALID_EMAIL)
        );
    });
    it('Malformated email to throw validation error', async () => {
        await expect(UserController.createUser({email:"testqovbzi", username:"test"})).rejects.toEqual(
            new BaseError("This email is invalid", ErrorCodes.INVALID_EMAIL)
        );
    });
    ...
    it('Existing username or email should throw an error', async () => {
        ...
        await expect(UserController.createUser({email:"test@email.com", username:"test"})).rejects.toEqual(
    ...
    it('When all good, user created', async () => {
        ...
        await expect(UserController.createUser({email:"test@email.com", username:"test"})).resolves.toEqual({
            username:"toto",
            email:"toto@email.com"
        });
    });
